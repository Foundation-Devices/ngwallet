...
mod tests {
    ...
    fn test_wallet() {
        ...
        let utxos = account.unspend_outputs();
        ...
        let utxos = account.unspend_outputs().unwrap_or(vec![]);
        if !utxos.is_empty() {
            let tag = "Test Tag".to_string();
            println!("\nSetting tag: {:?}", tag);
            let first_utxo = &utxos[0];
            account.set_tag(first_utxo, tag.clone());
            let utxos = account.unspend_outputs().unwrap_or(vec![]);
            ...
            account.set_do_not_spend(first_utxo, false);
